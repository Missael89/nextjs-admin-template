...
const Includes = () => {
    ...
    const filteredItems = includes.filter(
    ...
    const handleSumbitInsert = async (event) => {
        ...
        await axios.post('http://localhost:8080/api/includes/', jsonData)
            .then((response) => {
                if (response.data) {
    ...
    const handleSumbitUpdate = async (event) => {
        ...
        if (locale == 1) {
            await axios.post('http://localhost:8080/api/includes/', json)
                .then((response) => {
                    if (response.data) {
                        translations();
                    } else {
    ...
    const subHeaderComponentMemo = useMemo(() => {
        ...
        return (
            <FilterComponent onFilter={e => setFilterText(e.target.value)} onClear={handleClear} filterText={filterText} onFilterName={"Include"} />
        );
    ...
    useEffect(() => {
        ...
        async function getAll() {
            await axios.get('http://localhost:8080/api/includes/')
                .then((response) => {
                    if (response.data) {
                        let rows = response.data;

                        setIncludes(includes = rows);
            ...
            <Row center="xs">
                <Col breakPoint={{ xs: 12, md: 6 }}>
                    <form onSubmit={handleSumbitInsert}>
                        <Card>
                            <header>Add Include</header>
                            <CardBody>
                                <Input >
                                    <input type="text" placeholder="Snack(s) / Drinks / Guide..." name='name' required />
                                </Input>
                                <Input fullWidth>
                                    <input type="text" placeholder="Includes Description" name='description' required />
                                </Input>
                                <Input >
            ...
            >
                <Box sx={style}>
                    <form onSubmit={handleSumbitUpdate}>
                        <Card>
                            <header>Edit Include</header>
                            <CardBody>
                            <Input >
                                    <input type="text" placeholder="Snack(s) / Drinks / Guide..." name='name' value={name} onChange={handleName} required />
                                </Input>
                                <Input fullWidth>
                                    <input type="text" placeholder="Includes Description" name='description' value={description} onChange={handleDescription} required />
                                </Input>
                                <Input >
