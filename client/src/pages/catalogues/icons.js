...
const Icons = () => {
    ...
    const filteredItems = icons.filter(
        item => item.icon && item.icon.toLowerCase().includes(filterText.toLowerCase()),
    );
    ...
    const handleSumbit = async (event) => {
        ...
        await axios.post('http://localhost:8080/api/icons/', jsonData)
            .then((response) => {
                if (response.data) {
                    event.target.reset();
                    setReloadData(true);
    ...
    const handleButtonClick = (event) => {
        setIconId(iconId = event.id);
        setIconName(iconName = event.icon);
        setOpen(true)
    }

    const handleButtonDelete = async (event) => {
        await axios.delete(`http://localhost:8080/api/icons/${event.id}`)
            .then((res) => {
                        ...
                        <Card>
                            ...
                            <CardBody>
                                <Input fullWidth size="Small">
                                    <input type="text" placeholder="" name='icon' required />
                                </Input>
                            </CardBody>
                            <CardFooter>
            ...
            >
                <Box sx={style}>
                    <form onSubmit={handleSumbitUpdate}>
                        <Card>
                            <header>Icon Edit</header>
                            <CardBody>
                                <Input fullWidth size="Small">
                                    <input type="text" placeholder="" name='icon' value={iconName} onChange={handleIconName} required />
                                </Input>
                            </CardBody>
                            <CardFooter>
                                <Button appearance={`outline`} status="Success" type="submit">
