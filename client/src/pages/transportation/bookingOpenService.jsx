...
const BookingOpenService = () => {
    let [bookingOpenService, setBookingOpenService] = useState([]);
    ...
    const filteredItems = bookingOpenService.filter(
    ...
    const handleSumbitInsert = async (event) => {
        ...
        await axios.post('http://localhost:8080/api/bookingOpenService/', jsonData)
            .then((response) => {
                if (response.data) {
    ...
    const columns = useMemo(() => [
        ...
        {
            name: 'Transportation - Open Service',
            selector: (row) => row[5],
        }
    ], []);
    ...
    const subHeaderComponentMemo = useMemo(() => {
        ...
        return (
            <FilterComponent onFilter={e => setFilterText(e.target.value)} onClear={handleClear} filterText={filterText} onFilterName={"Open Service"} />
        );
    ...
    useEffect(() => {
        ...
        async function getAll() {
            await axios.get('http://localhost:8080/api/bookingOpenService/getAll')
                .then((response) => {
                    if (response.data) {
                        let rows = response.data;

                        setBookingOpenService(bookingOpenService = rows);
        ...
        <Layout title="Input">
            <Row center="xs">
                <Col breakPoint={{ xs: 12, md: 6 }}>
                    <form onSubmit={handleSumbitInsert}>
                        <Card>
                            <header>Add Transportation - Open Service</header>
                            <CardBody>
                                <Input fullWidth>
                                    <input type="text" placeholder="Cancún (4hrs)" name='name' id='name' required />
                                </Input>
                            </CardBody>
                            <CardFooter>
            ...
            <Row>
                <Col breakPoint={{ xs: 12 }}>
                    <Card>
                        <header>Transpotation - Open Service List</header>
                        <CardBody>
                            <DataTable
                                columns={columns}
                                data={filteredItems}
            ...
            >
                <Box sx={style}>
                    <form onSubmit={handleSumbitUpdate}>
                        <Card>
                            <header>Edit Transportation - Open Service</header>
                            <CardBody>
                                <Input fullWidth>
                                    <input type="text" placeholder="Cancún (4hrs)" name='nameIdentifier' value={name} onChange={handleName} required />
                                </Input>
                            </CardBody>
                            <CardFooter>
...
export default BookingOpenService;