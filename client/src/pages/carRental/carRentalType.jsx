...
const CarRentalType = () => {
  let [carRentalType, setCarRentalType] = useState([]);
  ...
  const filteredItems = carRentalType.filter(
  ...
  const handleSumbitInsert = async (event) => {
    ...
    await axios.post('http://localhost:8080/api/carRentalType/', jsonData)
      .then((response) => {
        if (response.data) {
  ...
  useEffect(() => {
    ...
    async function getAll() {
      await axios.get('http://localhost:8080/api/getCarRentalType')
        .then((response) => {
          if (response.data) {
            let rows = response.data;

            setCarRentalType(carRentalType = rows);
    ...
    <Layout title="Input">
      <Row center="xs">
        <Col breakPoint={{ xs: 12, md: 6 }}>
          <form onSubmit={handleSumbitInsert}>
            <Card>
              <header>Add Car Rental Type</header>
              <CardBody>
                <Input fullWidth>
      ...
      <Row>
        <Col breakPoint={{ xs: 12 }}>
          <Card>
            <header>Car Rental Type List</header>
            <CardBody>
              <DataTable
                columns={columns}
                data={filteredItems}
      ...
      >
        <Box sx={style}>
          <form onSubmit={handleSumbitUpdate}>
            <Card>
              <header>Edit Car Rental Type</header>
              <CardBody>
                <Input fullWidth>
