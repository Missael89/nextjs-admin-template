...
const CarRentalLocations = () => {
  let [carRentalLocations, setCarRentalLocations] = useState([]);
  ...
  const filteredItems = carRentalLocations.filter(
  ...
  const handleSumbitInsert = async (event) => {
    ...
    await axios.post('http://localhost:8080/api/carRentalLocations/', jsonData)
      .then((response) => {
        if (response.data) {
  ...
  const subHeaderComponentMemo = useMemo(() => {
    ...
    return (
      <FilterComponent onFilter={e => setFilterText(e.target.value)} onClear={handleClear} filterText={filterText} onFilterName={"Location"} />
    );
  ...
  useEffect(() => {
    ...
    async function getAll() {
      await axios.get('http://localhost:8080/api/getCarRentalLocations')
        .then((response) => {
          if (response.data) {
            let rows = response.data;

            setCarRentalLocations(carRentalLocations = rows);
    ...
    <Layout title="Input">
      <Row center="xs">
        <Col breakPoint={{ xs: 12, md: 6 }}>
          <form onSubmit={handleSumbitInsert}>
            <Card>
              <header>Add Car Rental Location</header>
              <CardBody>
                <Input fullWidth>
                  <input type="text" placeholder="AVIS (Aeropuerto - Cancún)" name='name' id='name' required />
                </Input>
              </CardBody>
              <CardFooter>
      ...
      <Row>
        <Col breakPoint={{ xs: 12 }}>
          <Card>
            <header>Car Rental Location List</header>
            <CardBody>
              <DataTable
                columns={columns}
                data={filteredItems}
      ...
      >
        <Box sx={style}>
          <form onSubmit={handleSumbitUpdate}>
            <Card>
              <header>Edit Car Rental Location</header>
              <CardBody>
                <Input fullWidth>
                  <input type="text" placeholder="AVIS (Aeropuerto - Cancún)" name='nameIdentifier' value={name} onChange={handleName} required />
                </Input>
              </CardBody>
              <CardFooter>
