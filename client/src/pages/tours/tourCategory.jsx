...
const TourCategory = () => {
    let [tourCategory, setTourCategory] = useState([]);
    ...
    const filteredItems = tourCategory.filter(
    ...
    const handleSumbitInsert = async (event) => {
        ...
        await axios.post('http://localhost:8080/api/tourCategory/', jsonData)
            .then((response) => {
                if (response.data) {
    ...
    const subHeaderComponentMemo = useMemo(() => {
        ...
        return (
            <FilterComponent onFilter={e => setFilterText(e.target.value)} onClear={handleClear} filterText={filterText} onFilterName={"Tour Category"} />
        );
    ...
    useEffect(() => {
        ...
        async function getAll() {
            await axios.get('http://localhost:8080/api/tourCategory/getAll')
                .then((response) => {
                    if (response.data) {
                        let rows = response.data;

                        setTourCategory(tourCategory = rows);
        ...
        <Layout title="Input">
            <Row center="xs">
                <Col breakPoint={{ xs: 12, md: 6 }}>
                    <form onSubmit={handleSumbitInsert}>
                        <Card>
                            <header>Add Tour Category</header>
                            <CardBody>
                                <Input fullWidth>
                                    <input type="text" placeholder="VIP / Group / LGBTI..." name='name' id='name' required />
                                </Input>
                            </CardBody>
                            <CardFooter>
            ...
            <Row>
                <Col breakPoint={{ xs: 12 }}>
                    <Card>
                        <header>Tour Category List</header>
                        <CardBody>
                            <DataTable
                                columns={columns}
                                data={filteredItems}
            ...
            >
                <Box sx={style}>
                    <form onSubmit={handleSumbitUpdate}>
                        <Card>
                            <header>Edit Tour Category</header>
                            <CardBody>
                                <Input fullWidth>
                                    <input type="text" placeholder="VIP / Group / LGBTI..." name='nameIdentifier' value={name} onChange={handleName} required />
                                </Input>
                            </CardBody>
                            <CardFooter>
